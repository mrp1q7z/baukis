...
require 'support/features_spec_helper'
...
feature '職員による顧客管理' do
  ...
  scenario '職員が顧客、住宅住所、勤務先を登録する' do
    # Given Section
    click_link '顧客管理'
    ...
    expect(new_customer.family_name).to eq('試験')
    expect(new_customer.given_name).to eq('花子')
    expect(new_customer.family_name_kana).to eq('シケン')
    expect(new_customer.given_name_kana).to eq('ハナコ')
    ...
    expect(new_customer.home_address.prefecture).to eq('東京都')
    expect(new_customer.home_address.city).to eq('千代田区')
    expect(new_customer.home_address.address1).to eq('千代田1-1-1')
    expect(new_customer.home_address.address2).to eq('')
    expect(new_customer.work_address.company_name).to eq('テスト')
    expect(new_customer.work_address.division_name).to eq('')
    expect(new_customer.work_address.postal_code).to eq('')
    expect(new_customer.work_address.prefecture).to eq('')
    expect(new_customer.work_address.city).to eq('')
    expect(new_customer.work_address.address1).to eq('')
    expect(new_customer.work_address.address2).to eq('')
  end

  scenario '職員が顧客、住宅住所、勤務先を更新する' do
    # Givenセクション（シナリオの前提条件）
    click_link '顧客管理'
