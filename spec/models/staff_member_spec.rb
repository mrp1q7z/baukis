...
describe StaffMember do
  ...
  describe '値の正規化' do
    ...
    example 'emailに含まれる全角英数記号を半角に変換' do
    ...
    example 'family_name_kanaに含まれる半角カナを全角カナに変換' do
      member = create(:staff_member, family_name_kana: 'ﾊﾝｶｸ')
      expect(member.family_name_kana).to eq('ハンカク')
    end
  end
  ...
  describe 'バリデーション' do
    ...
    example '長音を含むfamily_name_kanaは有効' do
